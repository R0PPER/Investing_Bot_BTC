...
def fetch_btc_binance() -> pd.DataFrame:
    ...
    try:
        params = {
            ...
            "interval": "1d",
            "limit": 500,   # ~500 ημέρες αρκούν για ATR warmup + Supertrend
        }
...
def execute_close_position(state: dict, price: float, date: datetime, reason: str) -> tuple[str, float]:
    ...
    # Αν κλείνουμε short λόγω MVRV+RSI → ενεργοποίηση overrides
...
def main() -> int:
    ...
    # ── Idempotency: skip αν έχει ήδη τρέξει σήμερα ──────────────────
    ...
    # ── Get yesterday's (last closed) candle ──────────────────────────
    # Signals si calcolano sulla barra precedente (close del giorno prima)
    ...
    prev_bar = signal_df.iloc[-2]    # ieri — barra che ha generato il segnale
    curr_bar = signal_df.iloc[-1]    # oggi — prezzo di fill (open)

    long_signal   = bool(prev_bar["long_signal"])
    short_signal  = bool(prev_bar["short_signal"])
    fill_price    = float(curr_bar["open"])    # fill al prezzo di apertura di oggi
    current_price = float(curr_bar["close"])   # prezzo attuale per calcoli
    ...
    # ══════════════════════════════════════════════════════════════════
    # TRADING DECISIONS (σειρά ίδια με leverage_bot03.py)
    # ══════════════════════════════════════════════════════════════════
    actions_taken = []
